...
cricket\tevaluate            imgur            open         run          tell      '''
        ...
        self.speaker_logo_disactivated = pygame.image.load("images/speakerOff.png").convert()
        ...
        self.Jarvis_title_main_window = self.Jarvis_title_main_window_font.render(self.screen_title, True, (31 ,184, 251))
        self.title_opening = self.title_opening_font.render(self.screen_title, True, (31 ,184, 251))
        ...
        self.events_list = [pygame.K_CLEAR, pygame.K_PAUSE, pygame.K_F1  , pygame.K_F2 , pygame.K_F3 , pygame.K_F4 , pygame.K_F5, pygame.K_F6, 
                   ...
                   pygame.K_RSUPER , pygame.K_MODE , pygame.K_HELP, pygame.K_INSERT, pygame.K_PAGEUP , pygame.K_PAGEDOWN, 
                   pygame.K_QUOTE, pygame.K_PRINT, pygame.K_SYSREQ , pygame.K_BREAK , pygame.K_MENU , pygame.K_POWER , pygame.K_EURO]
    ...
    def click_on_speaker(self, mouse_x, mouse_y):
        if mouse_x >= self.speaker_logo_coords[0] and mouse_x <= (self.speaker_logo_coords[0] + self.speaker_logo_coords[2]):
            return mouse_y >= self.speaker_logo_coords[1] and mouse_y <= (self.speaker_logo_coords[1] + self.speaker_logo_coords[3])  

    # Get if the mouse is on the delete button
    def click_on_delete(self, mouse_x, mouse_y):
        if mouse_x >= self.delete_logo_coords[0] and mouse_x <= (self.delete_logo_coords[0] + self.delete_logo_coords[2]):
            return mouse_y >= self.delete_logo_coords[1] and mouse_y <= (self.delete_logo_coords[1] + self.delete_logo_coords[3])
        ...
        for event in pygame.event.get():
            if event.type == QUIT or (event.type == KEYDOWN and event.key == K_ESCAPE):
                if main_screen:
                    ...
                    self.in_new_win = False
                    self.index_answer = 0 # to reset the index of the first element from 'self.answer_input_box.answers'
                ...
                if self.user_input_box.input_box_active: 
                    if event.key == pygame.K_RETURN:  
                        ...
                        if len(self.user_input_box.user_entry.split()):
                            self.user_input_box.history.insert(0, self.user_input_box.user_entry)
                        self.user_input_box.color_input_box = self.user_input_box.color_inactive_input_box 
                        ...
                        self.user_input_box.user_entry = ''
                        self.user_input_box.text_input_box = self.user_input_box.text_input_box_font.render(self.user_input_box.user_entry, True, self.answer_input_box.font_color)
                        ...
                        if cmd:
                            ...
                            ansi_escape = re.compile(r'\x1B\[[0-?]*[ -/]*[@-~]') # A regex for special characters
                            ...
                            # (when the answer takes too much space)
                                ...
                                self.answer_input_box.resize_box(self.screen, text[:-1], self.speaker_logo_coords[2], True)
                                ret.close()
                                return launch_empty_window 

                            self.answer_input_box.resize_box(self.screen, text[:-1], self.speaker_logo_coords[2], False)
                            ret.close()
                        else:
                            del self.answer_input_box.answers[:]
                            ...
                            self.answer_input_box.input_box.y = self.answer_input_box.coords_input[1]
                            self.answer_input_box.input_box.h = self.answer_input_box.coords_input[3]
                            self.answer_input_box.resize_box(self.screen, answer, self.speaker_logo_coords[2], False)
                            
                    # If we press the key '\b' (backspace), we delete a caracter
                    elif event.key == pygame.K_BACKSPACE:
                        ind = self.user_input_box.index_c
                        self.user_input_box.index_c -= 1 if (self.user_input_box.index_c - 1) >= 0 else 0
                        if ind > 0:
                            new_text = self.user_input_box.user_entry[:ind - 1] + '|' + self.user_input_box.user_entry[ind + 1: ] 
                            self.user_input_box.user_entry = new_text#self.user_input_box.user_entry[:-1]
                        self.user_input_box.text_input_box = self.user_input_box.text_input_box_font.render(self.user_input_box.user_entry, True, self.font_color)    
                    # If we press on delete, we delete the 
                    # character posionned after the cursor '|'    
                    elif event.key ==  pygame.K_DELETE :
                        ind = self.user_input_box.index_c
                        new_text = self.user_input_box.user_entry[:ind + 1] + self.user_input_box.user_entry[ind + 2:] 
                        self.user_input_box.user_entry = new_text
                        self.user_input_box.text_input_box = self.user_input_box.text_input_box_font.render(self.user_input_box.user_entry, True, self.font_color)    
                    # When press on tab, we display the possible
                    # completions (works only on commands)
                    elif event.key == pygame.K_TAB:
                        commands = self.completion(self.user_input_box.user_entry)
                        text = "   ".join(commands)
                        del self.answer_input_box.answers[:]
                        self.answer_input_box.resize_box(self.screen, text, self.speaker_logo_coords[2], False)
                    ...
                    # We position the cursor at the end         
                        ...
                        self.user_input_box.index_c = len(self.user_input_box.user_entry) - 1 # the length without the cursor   
                        self.user_input_box.render_cursor(self.screen)
                    ...
                    elif self.user_input_box.check_text_within_box() and not event.key in self.events_list:
                        #self.user_input_box.user_entry += event.unicode
                        ind = self.user_input_box.index_c
                        self.user_input_box.index_c += 1
                        text = self.user_input_box.user_entry 
                        text = self.user_input_box.insert_char(text, event.unicode, ind)
    ...
    def get_nb_lines(self, text):
        text = self.answer_input_box.ameliorate_text(text,  self.speaker_logo_coords[2])
        return self.answer_input_box.get_nb_lines(text, self.speaker_logo_coords[2])
    ...
    def scroll_up(self, up):        
        if up and (self.index_answer - 1) >= 0:
            ...
            limit = len(self.answer_input_box.answers) - (self.max_nb_lines + self.index_answer + 1)          
            if limit and len(self.answer_input_box.answers) > self.max_nb_lines:
                self.index_answer += 1              
            ...
            if (pygame.time.get_ticks() - t0) < 2000: #4000: 
                ...
                if action == launch_info_window:
                    ...
                    while not self.quit_empty_window:
                        self.user_input_box.input_box_active = True
                        self.event_listener(False)
                        self.cur_answers = self.text_scrolled() # to get only the text that can be displayed in the screen
                        self.screen.fill(self.screen_background_color)
                        self.answer_input_box.draw(self.screen, self.cur_answers, True, False)                             
                        pygame.display.update()
                        clock.tick(clock_tick_rate)
                    ...
                    self.answer_input_box.input_box.y = self.answer_input_box.coords_input[1]
                    self.answer_input_box.input_box.h = self.answer_input_box.coords_input[3]
                    self.answer_input_box.resize_box(self.screen, "What can I do for you", self.speaker_logo_coords[2], False)
                    self.answer_input_box.draw(self.screen, self.cur_answers,True, True)
                    ...
                    self.screen.blit(self.delete_logo, [self.delete_logo_coords[0], self.delete_logo_coords[1]])
                    self.screen.blit(self.speaker_logo, [self.speaker_logo_coords[0], self.speaker_logo_coords[1]])
