...
def main(self, s):
    ...
    try:
        ...
        for cnt, day_dict in enumerate(j['list']):
            ...
            print_say("\tWeather: {}".format(day_dict['weather'][0]['main']), self, Fore.BLUE)
            print_say(
                "\tMax temperature: {} {}".format(round(day_dict['temp']['max'], 1), units['str_units']),
                self,
                Fore.BLUE
            )
            print_say(
                "\tMin temperature: {} {}\n".format(round(day_dict['temp']['min'], 1), units['str_units']),
                self,
                Fore.BLUE
            )
    except KeyError:
