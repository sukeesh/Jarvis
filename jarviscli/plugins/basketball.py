...
class basketball():
    ...
    def get_headers(self):
        return {"x-rapidapi-host": "api-basketball.p.rapidapi.com", "x-rapidapi-key": APIKEY}
    ...
    def list_leagues(self, jarvis):
        ...
        if response is None:
            jarvis.spinner_stop(
                "Error While Loadin Data - Try Again Later.", Fore.YELLOW)
            return
        ...
        for i in range(total_count):
            print(" {}. {} {}".format(
                i + 1, leagues[i]["country"]["name"], leagues[i]["name"]))
    ...
    def search_request(self, jarvis, query, search_name):
        ...
        while len(value.strip()) < 3:
            print()
            print("The Search {} must be at least 3 characters in length",
                  search_name.lower())
            value = jarvis.input(
                "Enter {} Name: ".format(search_name), Fore.GREEN)
        jarvis.spinner_start('Searching...')
        response = self.fetch_data("{}?search={}".format(query, value.strip()))
        if response is None:
            jarvis.spinner_stop(
                "Error While Searching {} - Try Again Later.".format(search_name), Fore.YELLOW)
            return
        return response
    ...
    def search_team(self, jarvis):
        ...
        if found_count == 0:
            jarvis.spinner_stop(
                "Nothing was Found With Given Name", Fore.YELLOW)
            return
        ...
        for i in range(found_count):
            ...
            print(" {}. '{}' Country: {}".format(
                i + 1, team["name"], team["country"]["name"]))
        ...
        for i in range(found_count):
            ...
            if len(seasons) > 0:
                ...
                for j in range(len(seasons)):
                    print("   Season: {} Start: {} End: {}".format(
                        seasons[j]["season"], seasons[j]["start"], seasons[j]["end"]))
    ...
    def todays_games(self, jarvis):
        ...
        if response is None:
            jarvis.spinner_stop(
                "Error While Loading Matches - Try Again Later.", Fore.YELLOW)
            return
        total_count = response["results"]
        ...
        for i in range(total_count):
            ...
            print(" {}. {} {} {} {}".format(
                i + 1, country, league, time, teams))

    def get_option(self, jarvis):
        options = {1: "todays_games", 2: "search_team",
                   3: "list_leagues", 4: "search_league", 5: "new_key"}
    ...
    def get_choice(self, jarvis):
        while True:
            try:
                inserted_value = int(jarvis.input(
                    "Enter your choice: ", Fore.GREEN))
                if inserted_value == 6:
                    return -1
