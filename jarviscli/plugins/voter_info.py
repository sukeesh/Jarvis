...
def print_address(jarvis, location,  address):
    ...
    dateRange = "From " + \
        location["startDate"] + " to " + location["endDate"]
...
def get_voter_info(jarvis, s, address):
    ...
    if "election" not in voterInfo:
        ...
        jarvis.say(
            "Sorry, I wasn't able to find any election information for your location.", Fore.RED)
        return -1
    election_title_str = "Showing results for " + \
        str(voterInfo["election"]["name"]) + " on " + \
    ...
    if "dropOffLocation" in voterInfo:
        jarvis.say("----------------------------------------")
        jarvis.say(
            "There are absentee ballot drop boxes at these addresses", Fore.BLUE)
    ...
    if "otherElections" in voterInfo:
        ...
        for election in voterInfo["otherElections"]:
            election_title_str = str(election["name"]) + " on " + str(
                election["electionDay"]) + " id: " + str(election["id"])
            jarvis.say(election_title_str, Fore.RED)
        jarvis.say(
            "To get more information on these elections just say 'voterinfo' followed by the election id", Fore.BLUE)
...
def voter_info(jarvis, s):
    ...
    coordinates = str(current_location['latitude']) + \
    ...
    if elections != -1:
        jarvis.say(
            "More info has been stored in 'VoterInfo.json' in your current directory.", Fore.BLUE)
