...
def flightradar(jarvis, s):
    ...
    if option == 1:
        get_input_method = int(input(
            "How will you give the name of the airline:\n  1) By icao\n  2) By airline name\nPlease chose between(1, 2): "))
        if get_input_method == 1:
            airline_icao = input("What is the airlines icao: ")
            if airline_icao != "":
                airline_planes = flightapi.get_flights(
                    airline=airline_icao.upper())
                airline_flights = []
                for plane in airline_planes:
                    if plane.altitude != 0:
                        airline_flights.append(plane)
                jarvis.say("{:^5} {:^10} {:^20} {:^22} {:^10}   {:^10} {:^10} ".format(
                    "ICAO", "Registration", "Origin Airport", "Destination Airport", "latitude", "longitude", "time"))
                for flight in airline_flights:
                    jarvis.say("{:^5} {:^10}   {:^20} {:^22} {:^10}   {:^10}  {:^10}".format(flight.airline_icao, flight.registration,
                               flight.origin_airport_iata, flight.destination_airport_iata, flight.latitude, flight.longitude, flight.time))
            else:
            ...
            if run:
                ...
                for plane in airline_planes:
                    if plane.altitude != 0:
                        airline_flights.append(plane)
                jarvis.say("  {:^5}   {:^5} {:^10} {:^20} {:^22} {:^10}   {:^10} {:^10}".format(
                    "Name", "ICAO", "Registration", "Origin Airport", "Destination Airport", "latitude", "longitude", "time"))
                for flight in airline_flights:
                    jarvis.say("{:^5}  {:^5} {:^10}   {:^20} {:^22} {:^10}   {:^10}  {:^10}".format(airline_name, flight.airline_icao,
                               flight.registration, flight.origin_airport_iata, flight.destination_airport_iata, flight.latitude, flight.longitude, flight.time))
            else:
        ...
        get_input_method = int(input(
            "How will you give the name of the airports:\n  1) By iata\n  2) By airport name\nPlease chose between(1, 2): "))
        if get_input_method == 1:
            ...
            destination_airport_name = input(
                "What is the destination airport name: ")
            ...
            if origin_airport_name != destination_airport_name:
                ...
                if run:
                    ...
                    if not orun and not drun:
                        jarvis.say(
                            "Neither origin and destination airports were found")
                    elif not orun:
