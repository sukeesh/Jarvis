...
def browser(jarvis,s):
    ...
    If you want to open a new tab in an existing browser window, write 'browser new_tab'.
    (If there is no existing window, new window is opened.)
    """

    if "new_tab" in s:
        """Opens Google search in a new tab if a browser window is already opened, otherwise in new window."""
