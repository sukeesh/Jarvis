...
def weekday(jarvis, s):
    ...
    if a < 1:
        jarvis.say(
            "Sorry, but I only can the the day of the week from year 1 onwards! :(", Fore.RED)
        return

    if m < 1 or m > 12:
        jarvis.say(
            "Months go from 1 to 12! Remember to use dd/mm/aaaa format.", Fore.RED)
        return

    if d < 1 or d > 31:
        jarvis.say(
            "Days go from 1 to 31! Remember to use dd/mm/aaaa format.", Fore.RED)
        return
