...
def check_website_status (jarvis, s):
    ...
    while True:
        ...
        try:
            # tries to make a request to the URL that was input using defined headers that are not a "bot"
            headers = {}
            headers['User-Agent'] = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/69.0.3497.100 Safari/537.36"
            req = urllib.request.Request(url_request, headers=headers)
            page = urllib.request.urlopen(req)
            code = str(page.getcode())
            jarvis.say('The website ' + url_request + ' has returned a ' + code + ' code', Fore.BLUE)
            break
