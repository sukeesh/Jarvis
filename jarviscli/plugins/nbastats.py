...
class nbaStats():
    ...
    def __call__(self, jarvis):
        ...
        while True:
            ...
            stats = stats[['SEASON_ID', 'TEAM_ABBREVIATION', 'GP', 'MIN', 'PTS', 'REB', 'AST', 'STL', 'BLK', 'FG_PCT', 'FG3_PCT', 'FT_PCT']]
            ...
            while True:
                userRepeat = jarvis.input('Would you like to get more statistics? (y/n) ', Fore.GREEN)
                if userRepeat == 'y':
                    break
                elif userRepeat == 'n':
                    break
    ...
    def getUserStat(self, jarvis):
        statOptions = ['GP', 'MIN', 'PTS', 'REB', 'AST', 'STL', 'BLK', 'FG_PCT', 'FG3_PCT', 'FT_PCT']
        ...
        while True:
            ...
            # asks user for stat
            userStat = jarvis.input('What statistic would you like? ', Fore.GREEN)
            userStat = userStat.upper()
            if userStat in statOptions:
                return userStat
            else:
                print()
                jarvis.say('Please type a valid statistic. (ex. PTS)', Fore.YELLOW)
                print()
                time.sleep(0.3)
    ...
    def checkValidPlayer(self, jarvis):
        ...
        while True:
            playerName = jarvis.input('Please enter an NBA Player\'s name (be sure to capitalize accurately): ', Fore.GREEN)
            if playerName in playerNames:
                break
            else:
                jarvis.say('Player not found! Please enter a valid NBA player. Make sure spelling and capitalization is correct!', Fore.GREEN)
                print()
                time.sleep(0.3)
        return playerName
    ...
    def checkValidSeason(self, stats, jarvis):
        ...
        while True:
            season = jarvis.input('What year do you want stats from? (ex. 2022-23): ', Fore.GREEN)
            if season in seasonYears:
                break
            else:
                jarvis.say('Season not found! Please enter a valid season. (try checking formatting!)', Fore.YELLOW)
                print()
                time.sleep(0.3)
        return season
    ...
    def getSeasonRow(self, stats, season, jarvis):
        ...
        jarvis.say(f'This player has played for more than one team during the {season} season.', Fore.BLUE)
        while True:
            jarvis.say('You can pick the following teams:', Fore.BLUE)
            for team in rowTeams:
                print(rowTeams.get(team))
            teamChoice = jarvis.input('Pick a team (type TOT for total): ', Fore.GREEN)
