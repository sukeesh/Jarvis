...
def checkForWin(c):
    ...
    for i in range(numberRows):
        if whatsAtPos(i, c) != ' ':
            row = i
            if checkHorizWin(row, c, whatsAtPos(row, c)) or checkVertWin(row, c, whatsAtPos(row, c)) or checkDiagWin(row, c, whatsAtPos(row, c)):
                return True
            break
...
def game(jarvis, s):
    ...
    while playAgainFlag == 'y':
        ...
        while True:
            ...
            while checkIfFree(column) == False:
