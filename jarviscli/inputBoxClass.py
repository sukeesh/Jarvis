...
class inputBox(object):
    ...
    def __init__(self, x, y, width, height, text, font_size, color_active, color_inactive, font_color):
        ...
        self.text_input_box = self.text_input_box_font.render(text, True, font_color) 
        self.history = []
        self.user_entry = ''
    ...
    def check_text_within_box(self): 
        return (self.text_input_box.get_width() + 60) < (self.input_box.w + self.input_box.x)
    ...
    def move_cursor(self, screen, right):
        ...
        if right and (self.index_c + 1 <= length) and w < (self.input_box.w + self.input_box.x):
            self.index_c += 1
    ...
    def get_nb_lines(self, text, icon_width):
        max_length = self.coords_input[2] // 7 - icon_width // 8
        return len(text) // max_length + (1 if len(text) % max_length > 0 else 0) - 1
    ...
    def resize_box(self, screen, text, icon_width, empty_win):
        ...
        if nb_lines + 1 > 1:
            ...
            self.input_box.y = self.input_box.y  - (nb_lines + 1) * 15 # self.coords_input[3] / 4
    ...
    def draw(self, screen, answers, resize, rect):
        ...
        if resize: 
            ...
            for answer in answers:
                self.text_input_box = self.text_input_box_font.render(answer, True, self.font_color)
