...
class _Options(object):
    ...
    def __init__(self, **kwargs):
        ...
        for field in fields:
            value = kwargs[field] if field in kwargs else self._defaults.get(
                field)
    ...
    def getsuboptions(self, optclass):
        return {
            field: getattr(
                self,
                field) for field in optclass._allfields() if hasattr(
                self,
                field)}
...
# Options argument for the draw function. Specifies both text styling and
# positioning.
...
class _GetsurfOptions(_Options):
    ...
    def __init__(self, **kwargs):
        ...
        if self.shade:
            ...
            self.shade = 0
        self.ocolor = None if self.owidth is None else _resolvecolor(
            self.ocolor, DEFAULT_OUTLINE_COLOR)
        self.scolor = None if self.shadow is None else _resolvecolor(
            self.scolor, DEFAULT_SHADOW_COLOR)

        self._opx = None if self.owidth is None else ceil(
            self.owidth * self.fontsize * OUTLINE_UNIT)
        self._spx = None if self.shadow is None else tuple(
...
def getfont(**kwargs):
    ...
    if options.sysfontname is not None:
        ...
        try:
            ...
            raise IOError(
                "unable to read font filename: %s" %
                options.getfontpath())
...
def wrap(text, **kwargs):
    ...
    for jpara, para in enumerate(paras):
        ...
        # At any given time, a is the index of a known valid break point, and
        # line = para[:a].
        a = para.index(" ", lspaces) if " " in para[lspaces:] else len(para)
        line = para[:a]
        while a + 1 < len(para):
            # b is the next break point, with bline the corresponding line to
            # add.
            if " " not in para[a + 1:]:
                b = len(para)
                bline = para
            else:
                # Find a space character that immediately follows a non-space
                # character.
                b = para.index(" ", a + 1)
                while para[b - 1] == " ":
                    if " " in para[b + 1:]:
...
def _fitsize(text, size, **kwargs):
    ...
    def fits(fontsize):
        ...
        h = int(round((len(texts) - 1) * linesize +
                      texts[-1][1] * paraspace)) + font.get_height()
...
# number string, or an integer pixel value. See pygame.Color constructor
# for specification.
...
def _resolveangle(angle):
    if not angle:
        return 0
    angle %= 360
    return int(round(angle / ANGLE_RESOLUTION_DEGREES)) * \
...
def getsurf(text, **kwargs):
    ...
    if options.angle:
        ...
        _unrotated_size[(surf.get_size(), options.angle, text)
                        ] = surf0.get_size()
        ...
        surf0 = getsurf(text, **options.update(background=(0, 0,
                                                           0, 0), color=color, shadow=None, scolor=None))
        ssurf = getsurf(text, **options.update(background=(0, 0, 0, 0),
                                               color=options.scolor, shadow=None, scolor=None, gcolor=None))
        ...
        color = (0, 0, 0) if _istransparent(options.color) else options.color
        surf0 = getsurf(text, **options.update(color=color,
                                               ocolor=None, owidth=None))
        osurf = getsurf(text,
                        **options.update(color=options.ocolor,
                                         ocolor=None,
                                         owidth=None,
                                         background=(0,
                                                     0,
        ...
        # for background.
        if options.background is None or (len(
                options.background) > 3 and options.background[3] == 0) or options.gcolor is not None:
            lsurfs = [
                font.render(
                    text,
                    options.antialias,
                    color).convert_alpha() for text,
                jpara in texts]
        else:
            lsurfs = [
                font.render(
                    ...
                    options.antialias,
                    color,
                    options.background).convert_alpha() for text,
                jpara in texts]
        if options.gcolor is not None:
            gsurf0 = _gradsurf(
                lsurfs[0].get_height(),
        ...
        if len(lsurfs) == 1 and options.gcolor is None:
            ...
            ys = [int(round(k * linesize + jpara * parasize))
...
def layout(text, **kwargs):
    ...
    x0, y0 = _blitpos(options.angle, options.pos,
                      options.anchor, (sw, sh), None)
    ...
    if options.shadow is not None:
        spx, spy = (ceil(s * options.fontsize * SHADOW_UNIT)
                    for s in options.shadow)
