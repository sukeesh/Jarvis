...
class Recognizer(AudioSource):
    ...
    def recognize_google_cloud(self, audio_data, credentials_json=None, language="en-US", preferred_phrases=None,
        ...
        """
        assert isinstance(audio_data, AudioData), "``audio_data`` must be audio data"
        assert credentials_json is None or isinstance(credentials_json,
                                                      str), "``credentials_json_file_path`` must be ``None`` or a string"
        assert isinstance(language, str), "``language`` must be a string"
        ...
        if preferred_phrases is None:
            ...
            speech_config = {"encoding": "FLAC", "sampleRate": audio_data.sample_rate, "languageCode": language,
                             "speechContext": {"phrases": preferred_phrases}}
        request = speech_service.speech().syncrecognize(
    ...
    def recognize_bing(self, audio_data, key, language="en-US", show_all=False):
        ...
        request = Request(url, data=wav_data, headers={
            ...
            "Content-Type": "audio/wav; samplerate=16000; sourcerate={}; trustsourcerate=true".format(
