...
def mnist_classifier(s):
    ...
    try:
        ...
        print("Something went wrong during the loading of the image.")

    #Check if the shape of the image is right(28x28)
    if( not((np.shape(img)[0] == 28) and (np.shape(img)[1] == 28)) ):
        print("The image you gave me isn't Mnist type image.")
        return
    ...
    for row in range(28):
        for col in range(28):
            if( not(img[row][col][0] == img[row][col][1] and img[row][col][1] == img[row][col][2]) ):
    ...
    try:
        ...
        print("Something went wrong during the loading of my inteligence.")
    ...
    with torch.no_grad():
        ...
        print("According to my knowledge the image you gave me represents the digit {}.".format(predicted.item()))
...
#---------------------------run the code-------------------------------
try:        
    mnist_classifier("mnistTestImage_0.png")
except:
